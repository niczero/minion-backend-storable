...
subtest 'Repair missing worker' => sub {
  ...
  $guard->_workers->{$id}->{notified} = Time::HiRes::time - ($minion->missing_after + 1);
  undef $guard;
  $minion->repair;
...
subtest 'Repair stuck jobs' => sub {
  ...
  my $guard = $minion->backend->_guard->_write;
  $guard->_jobs->{$_}->{delayed} = Time::HiRes::time - ($minion->stuck_after + 1)
    for $id, $id2, $id3, $id4;
  undef $guard;
...
subtest 'List locks' => sub {
  ...
  $results = $minion->backend->list_locks(0, 1); # we sort locks by name
...
subtest 'List jobs' => sub {
  ...
  my $jobx = $minion->backend->list_jobs(0, 10)->{jobs};
  my $jobs = $minion->jobs;
  is $jobs->next->{task},      'add',  'right task';
  is $jobs->options->{before}, $jobx->[-1]->{id},      'before 1';
  ...
  is $jobs->next->{task},      'add',  'right task';
  is $jobs->options->{before}, $jobx->[-3]->{id},      'before 3';
  is $jobs->next->{task},      'fail', 'right task';
  is $jobs->options->{before}, $jobx->[-3]->{id},      'before 3';
  is $jobs->next->{task},      'fail', 'right task';
  is $jobs->options->{before}, $jobx->[-1]->{id},      'before 1';
  ...
  is $jobs->next->{task}, 'add', 'right task';
  is $jobs->total, 4, 'four jobs';
  $id2 = $minion->enqueue('addX');
  my $next = $jobs->next;
  is $next->{task}, 'fail', 'right task';
  my $before = $minion->backend->list_jobs(0, 1, {before => $next->{id}})->{jobs}->[0]->{id};
  ok $minion->job($before)->remove, 'job removed';
  is $jobs->total, 4, 'four jobs';
  is $jobs->next->{task}, 'fail', 'right task';
  is $jobs->next,  undef, 'no more results';
...
subtest 'Delayed jobs' => sub {
  ...
  my $guard = $minion->backend->_guard->_write;
  $guard->_jobs->{$id}->{delayed} = Time::HiRes::time - 24*60*60;
  undef $guard;
  ok $job = $worker->dequeue(0), 'job dequeued';
  is $job->id, $id, 'right id';
...
subtest 'Multiple attempts while processing' => sub {
  ...
  my $guard = $minion->backend->_guard->_write;
  $guard->_jobs->{$id}->{delayed} = Time::HiRes::time;
  undef $guard;
  ok $job = $worker->dequeue(0), 'job dequeued';
  is $job->id, $id, 'right id';
...
subtest 'Expiring jobs' => sub {
  ...
  is $minion->repair->jobs({states => ['inactive']})->total, 1, 'job has not expired yet';
  my $guard = $minion->backend->_guard->_write;
  $guard->_jobs->{$id}->{expires} = Time::HiRes::time - 24*60*60;
  undef $guard;
  is $minion->jobs({states => ['inactive']})->total, 0, 'job has expired';
  ok !$worker->dequeue(0), 'job has expired';
  ok exists $minion->backend->_guard->_jobs->{$id}, 'job still exists in database';
  $minion->repair;
  ok !exists $minion->backend->_guard->_jobs->{$id}, 'job no longer exists in database';
